...
opt.batch_size = 100
opt.train_size = 20000  -- set to 0 or 60000 to use all 60000 training data
opt.test_size = 0      -- 0 means load all data
opt.epochs = 6         -- **approximate** number of passes through the training data (see below for the `iterations` variable, which is calculated from this)
...
local n_test_data = test.data:size(1) -- number of training data
...
local feval_test = function(x)
  ...
  --          |  ...  |                 (except possibly for the last minibatch, we can't 
  --          --------                   let that one go past the end of the data, so we take a min())
  local start_indext = countert * opt.batch_size + 1
  local end_indext = math.min(n_test_data, (countert + 1) * opt.batch_size + 1)
  if end_indext == n_test_data then
    countert = 0
  else
    countert = countert + 1
  end

  local batch_inputst = test.data[{{start_indext, end_indext}, {}}]
  local batch_targetst = test.labels[{{start_indext, end_indext}}]
  gradParameters:zero()

  -- In order, these lines compute:
  -- 1. compute outputs (log probabilities) for each data point
  local batch_outputst = model:forward(batch_inputst)
  -- 2. compute the loss of these outputs, measured against the true labels in batch_target
  local batch_losst = criterion:forward(batch_outputst, batch_targetst)
...
local losses_test = {}          -- training losses for each iteration/minibatch
...
for i = 1, iterations do
  ...
  if i % 10 == 0 then -- don't print *every* iteration, this is enough to get the gist
      ...
      losses_test[#losses_test + 1] = feval_test(parameters)
...
gnuplot.plot({
  ...
  '-'},
{
  torch.range(1, #losses_test),
  torch.Tensor(losses_test),
  '-'}
)
------------------------------------------------------------------------------
...
for i = 1,classPredictions:size()[1] do
      if torch.all(torch.eq(classPredictions[i],test.labels[i])) then
            rate = rate + 1
      end
end

print('rate',rate/classPredictions:size()[1])
